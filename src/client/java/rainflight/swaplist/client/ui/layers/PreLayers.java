...
/**
 * This is a copy of owo's Layers, with render order tweaked to render components before render of the attached screen.
 * <p>
...
public final class PreLayers {

    /**
     * The event phase during which owo-ui layer instances are created and
     * initialized. This runs after the default phase
     */
    public static final Identifier INIT_PHASE = Swaplist.of("init-layers");

    private static final Multimap<Class<? extends Screen>, Layer<?, ?>> LAYERS =
            HashMultimap.create();
     ...
     */
    @SafeVarargs
    public static <S extends Screen, R extends ParentUIComponent> Layer<S, R> add(
        ...
        final var layer = new MyLayer<>(rootComponentMaker, instanceInitializer);
    ...
    public static <S extends Screen> List<Layer<S, ?>.Instance> getInstances(S screen) {
        return (List<Layer<S, ?>.Instance>)
                (Object) ((OwoScreenExtension) screen).owo$getInstancesView();
    }
    ...
    static {
        ...
        ScreenEvents.AFTER_INIT.register(
                ...
                (client, screeen, scaledWidth, scaledHeight) -> {
                    ...
                    ScreenEvents.beforeRender(screeen)
                            .register(
                                    (screen, context, mouseX, mouseY, tickDelta) -> {
                                        for (var instance : getInstances(screen)) {
                    ...
                    ScreenEvents.afterRender(screeen)
                            .register(
                                    (screen, context, mouseX, mouseY, tickDelta) -> {
                                        //                context.draw();
                                        for (var instance : getInstances(screen)) {
                                            instance.adapter.render(
                                                    context, mouseX, mouseY, tickDelta);
                                        }

                                        for (var instance : getInstances(screen)) {
                                            instance.adapter.drawTooltip(
                                                    context, mouseX, mouseY, tickDelta);
                                        }
                                    });

                    ScreenMouseEvents.allowMouseClick(screeen)
                            .register(
                                    (screen, click) -> {
                                        boolean handled;
                                        for (var instance : getInstances(screen)) {
                    ...
                    ScreenMouseEvents.allowMouseRelease(screeen)
                            .register(
                                    (screen, click) -> {
                                        boolean handled;
                                        for (var instance : getInstances(screen)) {
                    ...
                    ScreenMouseEvents.allowMouseScroll(screeen)
                            .register(
                                    (screen, mouseX, mouseY, horizontalAmount, verticalAmount) -> {
                                        boolean handled;
                                        for (var instance : getInstances(screen)) {
                                            handled =
                                                    instance.adapter.mouseScrolled(
                                                            mouseX,
                                                            mouseY,
                                                            horizontalAmount,
                                                            verticalAmount);
                    ...
                    ScreenKeyboardEvents.allowKeyPress(screeen)
                            .register(
                                    (screen, keyInput) -> {
                                        boolean handled;
                                        for (var instance : getInstances(screen)) {
                    ...
                    ScreenKeyboardEvents.allowKeyRelease(screeen)
                            .register(
                                    (screen, keyInput) -> {
                                        boolean handled;
                                        for (var instance : getInstances(screen)) {
